...
# Aims          : create RF model from predictors
...
if( ! file.exists( paste0(output_path, script_config_file ) ) ) {
  file.copy(from= paste0( script_config_dir, script_config_file) , 
            to= paste0(output_path, script_config_file ), 
...
# use only complete data points in RF model - either for full df or for the subset of static+climate data
# this ensures a common dataset is used across the div metrics (with the exception of the points not common between div or resilience metrics)
...
for (i in 1:length(v_optional_predictors)){ 
  ...
  # no diversity creates a model without a diversity metric
  ...
  # resample column to add randomness in the predictor for significance/p-value testing
  if(b_resample){
  ...
  if(b_do_testTrainSplit){ 
    # create test train split - # set the test/train split seed
    set.seed(n_setseed_trainTest) ; print( paste0('train-test seed: ', n_setseed_trainTest))   
    ...
    save(df_comb.train_i, file=paste0(output_path, 'df_comb.train_div-',var_name_i, '_targ-',
                                      target_name_j, '_seedTrainTest', n_setseed_trainTest, '_seedboot',
                                      n_setseed_boot, '.RData' )    )
    save(df_comb.test_i , file=paste0(output_path, 'df_comb.test_div-' ,var_name_i, '_targ-',
                                      target_name_j, '_seedTrainTest', n_setseed_trainTest, '_seedboot',
                                      n_setseed_boot, '.RData' )    )
    save(df_comb_i      , file=paste0(output_path, 'df_all_div-'       ,var_name_i, '_targ-',
                                      target_name_j, '_seedTrainTest', n_setseed_trainTest, '_seedboot',
                                      n_setseed_boot, '.RData' )    )
  ...
  # This variable makes the pdps within the function 
  # - in theory can probably make the pdps out of the rf instead of this (but this may be quicker)
  # for the no_diversity just set as any other variable (e.g. now set as forestcover)
  v_pdp <- ifelse(var_name_i == 'no_diversity' ,"forestcover" , var_name_i)
  print(paste0('investigating partial variable (v_pdp):', v_pdp))
  
  # set the values of the partial variable (ie the axis of the pdp) to predict on:
  # May not be neccessary as can also produce pdps from model
  pred_grid <- data.frame(v_pdp = seq(min(df_comb.train_i[[v_pdp]]), max(df_comb.train_i[[v_pdp]]),
                                      length.out = n_pdp_points))
  ...
  results <- foreach(i = 1:n_iterations, .packages = c("randomForest", "pdp"), .combine =
                       combine_function, .multicombine = TRUE) %dopar% {
                         boot_function(i)
                         }
  ...
  save(results     , file=paste0(output_path, 'list_rf_model_pdp_results_boot_parallel_nIter-',
                                 n_iterations, '_div-',var_name_i, '_seed-', n_setseed_boot, '_targ-',
                                 target_name_j, '.RData' )    ) 
