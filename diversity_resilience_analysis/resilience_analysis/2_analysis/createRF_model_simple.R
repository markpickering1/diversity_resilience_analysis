...
#                 NOTE: This version does not create bootstraps of the models
...
# Outputs	      : model
# Options	      : can run (simplified) cross validation on model parameters
# Date          : 2025-02-19
...
# initialise functions - unnecessary
...
for (i in 1:length(v_optional_predictors)){ 
  ...
  if(b_do_testTrainSplit){
    ...
    save(df_comb.train_i, file=paste0(output_path, 'df_comb.train_div-',var_name_i,
                                      '_targ-', target_name_j, '.RData' )    )
    save(df_comb.test_i , file=paste0(output_path, 'df_comb.test_div-' ,var_name_i, 
                                      '_targ-', target_name_j, '.RData' )    )
    save(df_comb_i      , file=paste0(output_path, 'df_all_div-'       ,var_name_i, 
                                      '_targ-', target_name_j, '.RData' )    )
  ...
  # run a (simplified) validation tuning on the dataset in order to optimise the parameter 
  if(b_run_tuning){
    # create CV parameters
    control <- trainControl(method="repeatedcv", number=k_fold_splits, 
                            repeats=k_fold_repeats, search="grid")
    formula_j <- as.formula(paste(target_name_j, "~ ."))
    # run CV 
    rf_gridsearch <- train(form = reformulate(termlabels = ".", response = target_name_j),
                           data=df_comb.train_i, method="rf", metric=metric, tuneGrid=tunegrid,
                           trControl=control)
    # save output
    save(rf_gridsearch, file=paste0(output_path, 'rf_tune_grid-',var_name_i, 
                                    '_targ-', target_name_j, '.RData' )    )
  ...
  save(rf.model, file=paste0(output_path, 'rf_model_div-', var_name_i, '_targ-', target_name_j, '.RData') ) 
