...
if(b_run_dice){ 
  ...
  for (k in 1:length(v_target)){ # k <- 1
    ...
    for (i in 1:length(v_optional_predictors)){ # i <- 1
      ...
      # 'createRF_bootDiv_metrics_preselectTestTrain_2024-03-03_noXYTS/list_of_rfs/list_rf_model_pdp_results_boot_parallel_nIter-20_div-shannon_entropy_seed-98_targ-kndvi_lambda_xt.RData'
      # load(s_name_rf_model) # rf.model - load the rf model for each div variable
      
      # this alternative loads the results[[x]] where x are different iterations
      ...
      save(df_dice, file=paste0(output_path, 'df_dice-', var_name_i, '_targ-', target_name_k, 'iter-', i_iter,  '.RData' )    ) # load(/eos/jeodpp/data/projects/FOREST-RESILIENCE/figures/rf_test_diversity/rf_test_diversity_2023-03-21_plot/df_ice_test-fhd_avg_diversity.RData)
