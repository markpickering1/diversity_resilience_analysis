# ########################################################
# Title         : plot_by_bgr_2d_pdp_and_isolines_bs.R
# Description   : run 2d pdp by bgr using all the bootstrapped rf model and for 
...
output_path <- paste0(root_data_figs, script_output_ext, '_2d_pdp_and_isolines_bs_', full_date,  '/')
...
for (i in 1:length(v_optional_predictors)){
  for (j in 1:length(v_target)){
    ...
    #bgr <- c(1) ; print(bgr) # try with one
    ...
    for (r in 1:length(bgr)){
      ...
      # create a list of temperature and diversity values between the average and the max (changed the increase in div from +10 to +3, and from -5 to -3)
      if (stats == 'mean') {
      ...
      for (l in 1:length(pdp_2d_extra_vars)){
        ...
        # create empty list to store the isolines file names
        isolines_list <- list()
        ...
        for(n in 1:length(l_seed) ){ 
          ...
          # initialise the file containing rf models
          input_file  <- paste0(input_dir_rf, 'list_rf_model_pdp_results_boot_parallel_nIter-20_div-' ,var_name_i, '_seed-', n_seed, '_targ-', target_i, '.RData' )
      
          # load the rf models
          load(input_file) 
          l_rf_boot <- results$rf.models; length(l_rf_boot)
          
          # now loop over the k different rf.models in the list for that seed and do the predictions
          for(k in 1:length(l_rf_boot)){ 
            ...
            # append the isoline to the isolines list
            isolines_list <- c(isolines_list, isoline_name)
        ...
        load(isolines_list[[1]])
        print(dim(isoline_df))
        isolines_merged <- isoline_df[, c('t2m_mean', var_name_i)]
        for (m in 2:length(isolines_list)){
          load(isolines_list[[m]])
          print(dim(isoline_df))
          isolines_merged <- merge(isolines_merged, isoline_df[, c('t2m_mean', var_name_i)], by = "t2m_mean", all = FALSE)
        }
        
        # calculate average and standard deviation of div_var values
        ...
        isoline_df_bs[, sd_div] <- apply(isolines_merged[div_columns], 1, sd)
        isoline_df_bs <- isoline_df_bs[, c('t2m_mean', mean_div, sd_div)]
        #isoline_df_bs[, target_i] <- target_res
