# ########################################################
# Title         : plot_by_bgr_pdp_bs.R
# Description   : run pdp by bgr with bs rf
    ...
    for (r in 1:length(bgr)){
      ...
      for(n in 1:1){
        ...
        for(k in 1:1) {
          ...
          # save pdp df
          save(pp_i, file=paste0(output_path, 'df_pdp-', var_name_i, '_', var_name_i, '_targ-', target_i, '_bgr-', bgr_region, '_seed-', n_seed, '_k-', k, '.RData'))
          
          # run the 1d pdp
          pp_i <- partialPlot(l_rf_boot[[k]], df_pdp_bgr_r, names(df_pdp_bgr_r)[10])
          
          # convert pdp to df and round digits
          pp_i <- as.data.frame(pp_i);  #pp_i[1:2] <- pp_i[1:2] %>% round( digits = 5)
          
          # change names of pdp df
          names(pp_i) <- c(names(df_pdp_bgr_r)[10], target_i)
          
          # save pdp df
          save(pp_i, file=paste0(output_path, 'df_pdp-', names(df_pdp_bgr_r)[10], '_', var_name_i, '_targ-', target_i, '_bgr-', bgr_region, '_seed-', n_seed, '_k-', k, '.RData'))
