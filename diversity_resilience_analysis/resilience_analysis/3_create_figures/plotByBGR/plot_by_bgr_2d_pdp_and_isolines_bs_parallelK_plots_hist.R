# ########################################################
# Title         : plot_by_bgr_2d_pdp_and_isolines_bs_parallelK_plots_hist.R
...
for (i in 1:length(v_optional_predictors)){
  for (j in 1:length(v_target)){
    ...
    merged_isolines_df <- merged_df %>% filter(t2m_mean >= (median_t2m_mean - 0.3) & !!sym(var_name_i) >= (!!sym(paste0('median_', var_name_i)) - 0.3 ))
    ...
    df_pdp_subset_d <- subset(df_pdp_bgr, get(var_name_i) >= ylims[1] & get(var_name_i) <= ylims[2])
    ...
    h_dist_div <- ggplot(df_pdp_subset_d, aes(x = df_pdp_subset_d[[var_name_i]], colour=factor(BiogeoRegions2016), fill=factor(BiogeoRegions2016))) +
      geom_density(alpha=0.5, aes(fill = factor(BiogeoRegions2016))) +
      scale_x_continuous(limits = ylims, breaks = seq(ylims[1], ylims[2], by = 0.2)) + #, expand = expansion(0, 0), labels = fixed_width_labels(12)) +
    ...
    # arrange plots
    g_isolines_hist <- grid.arrange(h_dist_div, NULL, g_isolines, legend_grob, NULL, h_dist_t2m, nrow = 2, ncol = 3, widths = c(1, 0.1, 3), heights = c(3, 1), top = paste0("Isolines of ", tlabs, " for each bs rf by bgr"))

    # save
    ggsave(plot = g_isolines_hist, filename = paste0(output_path_plots, 'df_pdp_2d_isolines_overlay-', var_name_i, '-t2m_mean_targ-', target_i, '.png'), width = 7, height = 5)
    ...
    # plot bs isolines with sd error band
    g_isoline_bs_sd <- ggplot(merged_isolines_df_bs, aes(x = t2m_mean, y = !!sym(paste0('mean_', var_name_i)), group = bgr, color = bgr)) +
    ...
    # extract the legend and remove
    # legend_grob <- cowplot::get_legend(g_isoline_bs_sd)
    g_isoline_bs_sd <- g_isoline_bs_sd + theme(legend.position = "none")
    
    # arrange plots
    g_isoline_bs_sd_hist <- grid.arrange(h_dist_div, NULL, g_isoline_bs_sd, legend_grob, NULL, h_dist_t2m, nrow = 2, ncol = 3, widths = c(1, 0.1, 3), heights = c(3, 1), top = paste0("Isolines of ", tlabs, " with sd error by bgr"))
    
    # save
    ggsave(plot = g_isoline_bs_sd_hist, filename = paste0(output_path_plots, 'df_pdp_2d_isolines_bs_sd-', var_name_i, '-t2m_mean_targ-', target_i, '.png'), width = 7, height = 5)
    
    # plot bs isolines with confidence interval (ci)
    g_isoline_bs_ci <- ggplot(merged_isolines_df_bs, aes(x = t2m_mean, y = !!sym(paste0('mean_', var_name_i)), group = bgr, color = bgr)) +
      geom_ribbon(aes(ymin = !!sym(paste0('ci_low_', var_name_i)),
                  ...
                  alpha = 0.2) +
      geom_line(linewidth = 0.25) +
      labs(x = "t2m (mean)", y = paste0(var_i_full, ' with CI')) +
      scale_y_continuous(limits = ylims, breaks = seq(ylims[1], ylims[2], by = 0.2)) + #, expand = expansion(0, 0), labels = fixed_width_labels(12)) +
      scale_x_continuous(limits = xlims, breaks = seq(xlims[1], xlims[2], by = 1)) + #, expand = expansion(0, 0)) +
      scale_color_manual(values = colors, labels = regions) +
    ...
    # arrange plots
    g_isoline_bs_ci_hist <- grid.arrange(h_dist_div, NULL, g_isoline_bs_ci, legend_grob, NULL, h_dist_t2m, nrow = 2, ncol = 3, widths = c(1, 0.1, 3), heights = c(3, 1), top = paste0("Isolines of ", tlabs, " with CI by bgr"))
    
    # save
    ggsave(plot = g_isoline_bs_ci_hist, filename = paste0(output_path_plots, 'df_pdp_2d_isolines_bs_ci-', var_name_i, '-t2m_mean_targ-', target_i, '.png'), width = 7, height = 5)
