...
# library(rgdal)        # no longer supported
...
for (i in 1:length(v_variables)){
  ...
  if(var_i =='kndvi'){
    file_i_deseason   <- paste0(root_data_input, input_kndvi_deseason)
    file_i_baseVar    <- paste0(root_data_input, input_kndvi_nodeseason)
  } else {
    file_i_deseason   <- paste0(root_data_input, v_variables[[var_i]] , var_i, residuals_ts_suffix, '.nc' ) 
...
###################################################
# open the static input ncdf files and extract 

# loop over the static variables
...
for (j in 1:length(v_variables_static)){ 
  ...
  file_i_baseVar <- paste0(root_data_input, v_variables_static[[v_variable_j]]  ) 
  
  # extract file type from right of full stop
  file_type <- strsplit(file_i_baseVar, "[.]")[[1]][2]   

  if (file_type == 'nc'){
    # if file netcdf format - repeat process from above
    r_i_baseVar    <- rast(file_i_baseVar) 
    r_i_baseVar_kndvi    <- rast(paste0(root_data_input, input_kndvi_nodeseason) )
    ...
    # diversity metrics may not be in not in lon lat format
    # first check same format as kNDVI data 
    if(crs(r_i_baseVar, proj = T) != crs(r_i_baseVar_kndvi, proj = T)){
    ...
    # if file in tif format and crs doesn't match - need to load then resample to fit resilience metric (i.e. v_variables[1])
    r_i_baseVar <- rast(file_i_baseVar)
    r_i_baseVar_kndvi     <- rast(paste0(root_data_input, input_kndvi_nodeseason))  #  plot(r_i_baseVar_kndvi[[1]])
  ...
  #   names(r_i_baseVar_sd) <- paste0('sd_', text_ba
  ...
  if( length(names(df_var)) == 3 ) { names(df_var)[3] <- v_variable_j
    ...
    colnames(df_var)[3:length(df_var)] <- paste0(v_variable_j, '_', colnames(df_var)[3:length(df_var)]) 
  }
