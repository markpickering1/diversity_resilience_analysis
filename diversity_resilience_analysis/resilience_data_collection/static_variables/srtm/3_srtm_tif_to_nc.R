# ########################################################
# Title         : 2_srtm_tif_to_nc.R
# Description   : Convert a given tif file to nc file
# Aims          : Convert the socc tif in nc file
# Inputs	      : SRTM tiff file
# Outputs	      : SRTM nc file
# Date          : 01/08/2023
# Version       : 3
...
######     GLOBAL VARS                        #####
main_dir <- 'static_variables/srtm/'
script_info <- '2_srtm_tif_to_nc'              
script_info_input <- 'SRTM_005'
script_info_output <- 'SRTM_005_nc'   
file_name <- 'slope_mean' 
variable <- 'slope_mean'
input_script_date <- '2023-08-01' 
...
if(nlat != 732 || nlon != 1109){ print(paste("NOTE: the file has ", nlon,' x ', nlat ," variables"))  }

# read resolution of raster object and verify correctness
res <- xres(r)
if(res != 0.05) { print(paste("NOTE: the file has ", res, "resolution"))  }
...
array_res <- 0.05 # hardcoded

# create lon/lat array of nc file - add half pixel for centre/bottomcorner adjustment 0.005/2
centre_corner_adjustment <- 0.05/2
lon <- as.array(seq(-10.66175 + centre_corner_adjustment, 44.78825 + centre_corner_adjustment - array_res, array_res)) 
lat <- as.array(seq( 34.58425  + centre_corner_adjustment, 71.18425 + centre_corner_adjustment - array_res, array_res)) 
