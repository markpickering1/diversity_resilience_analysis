...
######     GLOBAL VARS                        #####
script_name  <- '3_ssr_shift_leapY.R'           # name of the script
script_info  <- '3_ssr_shift_leapY'           # first attempt
variable_i   <- 'ssr'
variable_i_ncName <- 'ssr'
...
input_path   <- paste0('ssr/3_ssr_extract_8day/unshifted/')
...
for(file_i in nc_files){ # file_i <- nc_files[1]
  ...
  # didn't work as warning not error
  # skip_next <- F
  # year <- tryCatch(
  #   as.numeric(file_i_year),
  #   error=function(e) {skip_next <<- T}
  # )
  # if(skip_next) {print('non-numeric year') ; next }
  ...
  # get units
  var_units <- override_units  # var_units <- ncin$var[[variable_i_ncName]][['units']] # ncatt_get(var_balance, 'balance' ,"units")
