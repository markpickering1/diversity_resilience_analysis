...
variable_i   <- 'tp'
variable_i_ncName <- 'tp'

time_Shift_factor <- -12 # shift this number of hours from all time values
override_units <- "mm"
...
# base paths - user insert paths
root_data   <- paste0(variable_i,'/')
input_path   <- paste0('tp/2_tp_extract_8day/')
# input_path  <- paste0(root_data, variable_i,'_1_separate/')
output_path     <- paste0(root_data, '3_', variable_i,'_align/leap_years/')
...
nc_files <- list.files(path=input_path, pattern=paste0("*nc"), full.names=FALSE, recursive=FALSE) # view all files

for(file_i in nc_files){ # file_i <- nc_files[2]
  ...
  file_i_year <- strsplit(file_i_name , '_')[[1]][4] # extract the year
  ...
  # apply a shift to all the time values (except the last to hardcode their match to other datasets) - 
  # this results from dropping the final day in the accumulation methods (which goes into next year)
  time_vals[1:length(time_vals)-1]      <- time_vals[1:length(time_vals)-1] + time_Shift_factor 
  ...
  if(is.na(file_i_year) || file_i_year %% 4 == 0 ) {
    # extract the index of the first date after 28th February
  ...
  lat <- ncvar_get(ncin, 'latitude' ) ; lon <- ncvar_get(ncin, 'longitude' )
  ...
  # get units
  var_units <- override_units  # ncin$var[[variable_i_ncName]][['units']] # ncatt_get(var_balance, 'balance' ,"units")
